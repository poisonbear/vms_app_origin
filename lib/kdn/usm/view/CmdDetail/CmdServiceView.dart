...
class CmdServiceView extends StatefulWidget {
  const CmdServiceView({super.key});

  @override
  State<CmdServiceView> createState() => _CmdViewState();
}

class _CmdViewState extends State<CmdServiceView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // 앱 상단 제목
      appBar: AppBar(
        title: const AppBarLayerView('회원가입2'),
        leading:  IconButton(
      ...
      body: Consumer<CmdServiceViewModel>( // 데이터를 불러 올 때는 Consumer 항상 viewmodel을 거쳐서 가져오기
